...
2. **🔌 Qwen 首发推荐，多模型统一接入** 
   默认并深度适配 **通义千问 (Qwen)**，同时原生支持设置挂载 DeepSeek, OpenAI, MiniMax 等 API Key。
3. **🪨 纯血本地优先 (Local-First)**
   没有任何远端同步阴谋。架构依托于 `better-sqlite3` 构建，底层强制实施数据库和本地 Markdown（`.md` 纯文本）的双写机制，你的读书笔记和高亮永远攥在自己硬盘里。
4. **📐 L0-L3 三维极简空间防塌陷 UI**
   采用严苛的视觉层级管控，AI 解释与划线悬浮菜单像水流一样依附于正文边缘，专为高密度长篇论文排版解构。
